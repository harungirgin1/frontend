# html
html-ilk-ödev

# yapılanlar
<h1>Abdullah Harun Girgin</h1>
<!-- açıklama yazınız-->
<h2> Hakkımda </h2> 
<p> Merhaba. 25 yaşındayım. </p>
<p>İnşaat mühendisiyim ancakl yazılım öğrenmeye başladım</p>
<p> Kendimi sürekli geliştiriyour</p>

 <!-- <h3>açıoklma kısmına al</h3> -->
 <h3>listeler</h3>
<!-- Sıralı listeler : 1, 2, 3, ordered list -- > ol   -->
<!--  Sıralı olmayan listeler -- > *, *, *, unordered list ? -- > ul -->
<!-- list item -- > li-->
<ol>
<li>liste öğesi 1
    <ol> 
            <li>liste öğesi 5</li>
            <li>liste öğesi 5</li>
            <li>liste öğesi 6</li>
        
        </ol>
</li>
<li>liste öğesi 2</li>
<li> diğer özellikler
    <ul> 
    <li>liste öğesi 1</li>
    <li>liste öğesi 2</li>
    <li>liste öğesi 3</li>
    
    </ul>

</li>

</ol>

<ol type="I">
<b>roma rakamları</b>  
    <li>hesaplamalar</li>
    <li>kelimeler</li>
    <li>cevaplar</li>


</ol>

<ol type="A">
    <b>harf sıralaması</b>  
        <li>hesaplamalar</li>
        <li>kelimeler</li>
        <li>cevaplar</li>
    
    
    </ol>

    <li> kelimeler</li>
    <li> kelimeler</li>
    <li> kelimeler</li>
    <li> kelimeler</li>

<h4> Emeet Zen coding </h4>
<!--    etiket isimlerini direk verebiliyoruz
+ kardeş öğe
> içinde öğe
^ yukarı çık ve öğe oluştur
* çarp -- > p*3
lorem ??
lorem4 -- > 4 kelimelik lorem üret
. -- > class
# id
-->
<!-- h1+p+p+h2+p+p+p-->
<!-- h1+p*2+h2+p*4-->
<!--  h1>li*3^p*2+h3-->
<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquid quidem, ab omnis aut repudiandae optio officiis accusantium magni fugit minus necessitatibus in porro pariatur nemo itaque tempore rerum asperiores sit.</p>

<p> Lorem, ipsum dolor.</p>

<!-- h3.deneme>lorem3-->
<h3 class="deneme">Lorem, ipsum dolor.</h3>

<!-- h3#idolussun>lorem3  -->
<h3 id="idolussun">Lorem, ipsum dolor.</h3>

<!-- h3.deneme#>lorem3-->
<h3 class="deneme" id="">Lorem, ipsum dolor.</h3>

<!-- h3.kodluyoruz.html-->
<h3 class="kodlujyoruz html"></h3>

<!--ul>li*3.text.tepki-->
<ul>
    <li class="text tepki"></li>
    <li class="text tepki"></li>
    <li class="text tepki"></li>
</ul>


<h1>görsellerle çalışmak</h1>
<!--    img-->

<img src="data:image/png;base64," alt="resim">

<img src="https://picsum.photos/id/559/600/300" alt="">

<img src="img/1.jpg" alt="resim" border="15" align="left" > 

<a href="http://www.girgintekstil.net/">
    <img src="https://picsum.photos/seed/picsum/536/354" alt="girgin">
</a>

<img src="https://i.picsum.photos/id/1047/600/300.jpg?hmac=mgNMZuYbfvqtt5TuRzAAnf4VVAonSOgIpEA1T2K4KJA" alt="resim" />

